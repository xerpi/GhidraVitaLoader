...
1. `$ arm-vita-eabi-gcc -P -E $VITASDK/arm-vita-eabi/include/vitasdkkern.h -D"__attribute__(x)=" -D"__extension__(x)=" > vita_kernel.h`
2. Now open `vita_kernel.h` and remove the `typedef unsigned int wchar_t;` (line 3)
3. Remove the `ksceKernelCpu*Context` inline macros
4. Change `SceKernelProcessInfo`'s `unk[0xE8 / 4 - 6]` to `unk[0x34]`

### 2. Parsing the header
1. Open Ghidra and go to _File_ -> _Parse C Source..._ and select `vita_kernel.h`
